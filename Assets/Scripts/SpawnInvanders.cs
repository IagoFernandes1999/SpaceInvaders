...
public class SpawnInvanders : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
             GameObject newInvader = Instantiate(invasorA, transform);
             newInvader.transform.position = new Vector3(x2, -1f, 0f);
             x2 += 1f;
        }
        float x3 = xMin;
        for (int i = 1; i <= nInvasores; i++)
        ...
        {
            GameObject newInvader = Instantiate(invasorB, transform);
            newInvader.transform.position = new Vector3(x4, 0f, 0f);
            x4 += 1f;
        }
        float x5 = xMin;
        for (int i = 1; i <= nInvasores; i++)
        {
            ...
            newInvader.transform.position = new Vector3(x5, 0.5f, 0f);
            x5 += 1f;
        }
